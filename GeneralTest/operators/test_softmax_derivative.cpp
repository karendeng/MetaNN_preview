...
void test_softmax_derivative2()
{
    ...
    auto mSout = GenMatrix<float>(4, 1, 1.0f, 0.0001f);
    auto mGrad = GenMatrix<float>(4, 1, .3f, 0.005f);
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            if (i == j)
            {
                helper.SetValue(i, j, mSout(i, 0) * (1 - mSout(i, 0)));
            }
            else
            {
                helper.SetValue(i, j, -mSout(i, 0) * mSout(j, 0));
            }
        }
    }
    helper = Evaluate(Dot(helper, mGrad));
    for (size_t i = 0; i < 4; ++i)
    {
        assert(fabs(t_r(i, 0) - helper(i, 0)) < 0.0001);
    }
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        assert(fabs(t_r(i, 0) - helper(i, 0)) < 0.001);
    }
    cout << "done" << endl;
}
...
void test_softmax_derivative3()
{
    ...
    {
        auto bSout = GenMatrix<float>(4, 1, 1.0f, 0.0001f);
        auto bGrad = GenMatrix<float>(4, 1, 0.3f, 0.007f);
        auto res = ColSoftmaxDerivative(bGrad, bSout);
        auto res2 = ColSoftmaxDerivative(bGrad, bSout);
    ...
    }
    {
        ...
        auto res = RowSoftmaxDerivative(bGrad, bSout);
        auto res2 = RowSoftmaxDerivative(bGrad, bSout);
...
void test_softmax_derivative4()
{
    ...
    {
        MakeLayer<SoftmaxLayer, PFeedbackOutput, PRowVecInput> layer1;
        MakeLayer<NegativeLogLikelihoodLayer, PFeedbackOutput, PRowVecInput> layer2;
    ...
    {
        MakeLayer<SoftmaxLayer, PFeedbackOutput> layer1;
        MakeLayer<NegativeLogLikelihoodLayer, PFeedbackOutput> layer2;

        auto layer1Input = LayerIO::Create().Set<LayerIO>(GenMatrix<float>(5, 1, 1, 1));
        auto layer1Output = layer1.FeedForward(layer1Input).Get<LayerIO>();

        auto target = GenMatrix<float>(5, 1, 0.1f, -0.3f);
        auto layer2Input = CostLayerIn::Create()
                                       .Set<CostLayerIn>(layer1Output)
                                       .Set<CostLayerLabel>(target);
        ...
        for (size_t i = 0; i < 5; ++i)
        {
            float compare = softRes(i, 0) * sum - target(i, 0);
            assert(fabs(check(i, 0) - compare * 0.7f) <= 0.0001);
        }
    }
    cout << "done" << endl;
}
