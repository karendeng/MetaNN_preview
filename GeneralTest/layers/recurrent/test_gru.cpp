...
void comp(const Batch<Matrix<float, DeviceTags::CPU>>& v1, const Batch<Matrix<float, DeviceTags::CPU>>& v2)
{
...
void test_gru_with_bptt()
{
    ...
    using GRUKernel = MakeLayer<RecurrentLayer, PUpdate, PFeedbackOutput, PRowVecInput>;
    using WeightKernel = MakeLayer<WeightLayer, PUpdate, PFeedbackOutput, PRowVecInput>;

    WeightKernel wi("wi", 4, 6);    // input
    WeightKernel wr("wr", 4, 6);    // reset
    WeightKernel wu("wu", 4, 6);    // update

    GRUKernel gru("trans", 6, 6);

    map<std::string, Matrix<float, DeviceTags::CPU>> params;
    params["wi"] = Matrix<float, DeviceTags::CPU>(4, 6);
    params["wu"] = Matrix<float, DeviceTags::CPU>(4, 6);
    params["wr"] = Matrix<float, DeviceTags::CPU>(4, 6);
    params["trans-input"] = Matrix<float, DeviceTags::CPU>(6, 6);
    params["trans-reset"] = Matrix<float, DeviceTags::CPU>(6, 6);
    params["trans-update"] = Matrix<float, DeviceTags::CPU>(6, 6);
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        ...
        {
            params["wi"].SetValue(i, j, (float)(weight_input[id]));
            params["wu"].SetValue(i, j, (float)(weight_update[id]));
            params["wr"].SetValue(i, j, (float)(weight_reset[id]));
            ++id;
        }
    }
    ...
    for (size_t i = 0; i < 6; ++i)
    {
        for (size_t j = 0; j <6; ++j)
        {
            params["trans-input"].SetValue(i, j, trans_input[id]);
            params["trans-reset"].SetValue(i, j, trans_reset[id]);
            params["trans-update"].SetValue(i, j, trans_update[id]);
            ++id;
        }
    }

    wi.LoadWeights(params);
    wr.LoadWeights(params);
    wu.LoadWeights(params);
    gru.LoadWeights(params);

    Batch<Matrix<float, DeviceTags::CPU>> x0(1, 4, 2);
    Batch<Matrix<float, DeviceTags::CPU>> x1(1, 4, 2);
    Batch<Matrix<float, DeviceTags::CPU>> x2(1, 4, 2);
    id = 0;
    for (size_t i = 0; i < 2; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            x0[i].SetValue(0, j, x_np_0[id]);
            x1[i].SetValue(0, j, x_np_1[id]);
            x2[i].SetValue(0, j, x_np_2[id]);
            ++id;
        }
    }

    Batch<Matrix<float, DeviceTags::CPU>> fres0(1, 6, 2);
    Batch<Matrix<float, DeviceTags::CPU>> fres1(1, 6, 2);
    Batch<Matrix<float, DeviceTags::CPU>> fres2(1, 6, 2);
    id = 0;
    for (size_t i = 0; i < 2; ++i)
    {
        for (size_t j = 0; j < 6; ++j)
        {
            fres0[i].SetValue(0, j, fres_0[id]);
            fres1[i].SetValue(0, j, fres_1[id]);
            fres2[i].SetValue(0, j, fres_2[id]);
            ++id;
        }
    }
    ...
    vector<Batch<Matrix<float, DeviceTags::CPU>>> forward(3, {1, 6});
    
    for (size_t batchSize = 0; batchSize < 2; ++batchSize)
    {
        vector<TResData> fData;
        
        auto iw0 = LayerIO::Create().Set<LayerIO>(x0[batchSize]);
        auto wi_res0 = wi.FeedForward(iw0).Get<LayerIO>();
        auto wr_res0 = wr.FeedForward(iw0).Get<LayerIO>();
        auto wu_res0 = wu.FeedForward(iw0).Get<LayerIO>();

        auto gru0 = GRUKernel::InputType::Create()
                        .Set<GruInputBelow>(wi_res0)
                        .Set<GruUpdateBelow>(wu_res0)
                        .Set<GruResetBelow>(wr_res0)
                        .Set<RnnLayerHiddenBefore>(TrivalMatrix<float, DeviceTags::CPU>(1, 6, 1));

        fData.push_back(MakeDynamic(gru.FeedForward(gru0).Get<LayerIO>()));
        
        auto iw1 = LayerIO::Create().Set<LayerIO>(x1[batchSize]);
        auto wi_res1 = wi.FeedForward(iw1).Get<LayerIO>();
        auto wr_res1 = wr.FeedForward(iw1).Get<LayerIO>();
        auto wu_res1 = wu.FeedForward(iw1).Get<LayerIO>();

        auto gru1 = GRUKernel::InputType::Create()
                        .Set<GruInputBelow>(wi_res1)
                        .Set<GruUpdateBelow>(wu_res1)
                        .Set<GruResetBelow>(wr_res1);
                        
        fData.push_back(MakeDynamic(gru.FeedForward(gru1).Get<LayerIO>()));
        
        auto iw2 = LayerIO::Create().Set<LayerIO>(x2[batchSize]);
        auto wi_res2 = wi.FeedForward(iw2).Get<LayerIO>();
        auto wr_res2 = wr.FeedForward(iw2).Get<LayerIO>();
        auto wu_res2 = wu.FeedForward(iw2).Get<LayerIO>();

        auto gru2 = GRUKernel::InputType::Create()
                        .Set<GruInputBelow>(wi_res2)
                        .Set<GruUpdateBelow>(wu_res2)
                        .Set<GruResetBelow>(wr_res2);
        ...
        {
            forward[i].PushBack(fHandle[i].Data());
        }
    }
    cout << "\tCompare for forward 0: ", comp(forward[0], fres0);
    cout << "\tCompare for forward 1: ", comp(forward[1], fres1);
    cout << "\tCompare for forward 2: ", comp(forward[2], fres2);

    GradCollector<float, DeviceTags::CPU> grad_collector;
    gru.GradCollect(grad_collector);
    wi.GradCollect(grad_collector);
    wu.GradCollect(grad_collector);
    wr.GradCollect(grad_collector);
