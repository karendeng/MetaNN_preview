...
public:
    ...
    bool operator!= (const TBase& val) const
    {
        return !(operator==(val));
    }
...
struct DynamicType_
{
    using type = DynamicData<typename TData::ElementType,
                             typename TData::DeviceType,
                             DataCategory<TData>>;
};

template <typename TElem, typename TDevice, typename TDataCate>
struct DynamicType_<DynamicData<TElem, TDevice, TDataCate>>
{
    using type = DynamicData<TElem, TDevice, TDataCate>;
};
...
auto MakeDynamic(TData&& data)
{
    ...
    using resType = DynamicType<rawData>;
    return resType(std::move(baseData));
}
