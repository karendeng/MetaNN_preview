...
struct FeedbackOut_
{
    ...
    static auto FeedBack(DataType& data, const TGrad& grad)
    {
        ...
        auto& tmp2 = data.top();
        auto res = LayerIO::Create().template Set<LayerIO>(SigmoidDerivative(tmp, tmp2));
        data.pop();
        return res;
    }
};
...
private:
    using FeedbackOut_ = NSSigmoidLayer::FeedbackOut_<IsFeedbackOutput>;
