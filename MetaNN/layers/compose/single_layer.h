...
template <>
struct ActPick_<SingleLayerPolicy::ActionTypeEnum::Sigmoid>
{
    template <typename T>
...
template <>
struct ActPick_<SingleLayerPolicy::ActionTypeEnum::Tanh>
{
    template <typename T>
...
struct KernelHelper
{
    ...
    using ActType = typename ActPick_<typename PolicySelect<SingleLayerPolicy,
                                                            PlainPolicies>::ActionType>::template type<T>;
...
struct TupleCreator
{
    constexpr static bool hasBias = KernelHelper<TPolicies>::hasBias;

    template <bool checkRes, typename TDummy = void>
    struct imp
    {
        template <typename TupleProcessor>
        static auto Eval(const std::string& p_name, size_t p_inputLen, size_t p_outputLen)
        {
            return TupleProcessor::Create()
    ...
    static auto Eval(const std::string& p_name, size_t p_inputLen, size_t p_outputLen)
    {
        return imp<hasBias>::template Eval<TupleProcessor>(p_name, p_inputLen, p_outputLen);
    }
};
}
...
{
    using TBase = NSSingleLayer::template Base<TPolicies>;
...
public:
    SingleLayer(const std::string& p_name, size_t p_inputLen, size_t p_outputLen)
        : TBase(NSSingleLayer::TupleCreator<TPolicies>::template Eval<TupleProcessor>(p_name, p_inputLen, p_outputLen))
    { }
};
}
